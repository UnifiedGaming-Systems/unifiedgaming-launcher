...
param(
	...
	# Temp Dir Fir Build
	[string]$TempDir = (Join-Path $env:TEMP "UnifiedGL"),

	[string]$LicencedDependenciesUrl,
...
function PackExtensionTemplate()
{
    ...
    $tempFiles = Get-Content "..\source\Tools\UnifiedGL.Toolbox\Templates\Extensions\$TemplateRootName\BuildInclude.txt" | Where { ![string]::IsNullOrEmpty($_) }
    ...
    foreach ($file in $tempFiles)
    {
        ...
        Copy-Item (Join-Path "..\source\Tools\UnifiedGL.Toolbox\Templates\Extensions\$TemplateRootName" $file) $target        
    }
...
$platforms = Get-Content "..\source\UnifiedGL\Emulation\Platforms.yaml" -Raw | ConvertFrom-Yaml
...
$regions = Get-Content "..\source\UnifiedGL\Emulation\Regions.yaml" -Raw | ConvertFrom-Yaml
...
Get-ChildItem "..\source\UnifiedGL\Emulation\" -Filter "*.yaml" -Recurse | ForEach {
...
if (!$SkipBuild)
{
    ...
    if ($OnlineInstallerConfig)
    {
        ...
        $locaConfigPath = "..\source\Tools\UnifiedGLInstaller\installer_mirrors.txt"
    ...
    Invoke-Nuget "restore ..\source\UnifiedGL.sln"
...
if ($Package)
{
    ...
    $packageName = Join-Path $InstallerDir "UnifiedGL.zip"
    New-ZipFromDirectory $OutputDir $packageName
}

(Get-ChildItem (Join-Path $OutputDir "UnifiedGL.dll")).VersionInfo.FileVersion | Write-Host -ForegroundColor Green
return $true