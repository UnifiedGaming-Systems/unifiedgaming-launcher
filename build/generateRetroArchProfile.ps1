param(
    ...
    [string]$RetroArchDir,
    [Parameter(Mandatory=$true)]
    [string]$UnifiedGLSourceDirectory
)
...
$emulationDirPath = [System.IO.Path]::Combine($UnifiedGLSourceDirectory, "UnifiedGL", "Emulation")
...
{
    Write-Host "UnifiedGL platforms definition file not found in $platformsDefinitionPath" -ForegroundColor Yellow
    return
}
...
# Add RetroArch to emulators list in UnifiedGL platforms definitions
...
Write-Host "RetroArch emulator and UnifiedGL platforms definitions updated and saved successfully!" -ForegroundColor Green